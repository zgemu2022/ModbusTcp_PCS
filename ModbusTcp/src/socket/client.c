...
int createFun03Frame(int id_thread, int taskid,int *lenframe,unsigned char *framebuf)
{

	int numTask=ARRAY_LEN(pcsYc);
    Pcs_Fun03_Struct pcs=pcsYc[taskid];
	int pos=0;
...
static int doFun03Tasks(int id_thread,int taskid)
{
	int numfail=0;
	...
	taskid_temp = createFun03Frame(id_thread, taskid,&lensend,sendbuf);
...
void *Modbus_clientSend_thread(void *arg) // 25
{
	...
	while (modbus_sockt_state[id_thread]==STATUS_ON)//
	{
		...
		{
			 printf("do something!!!!\n");
           curTaskId = doFun03Tasks(id_thread,curTaskId);
		}
...
void CreateThreads(void)
{
	...
	for (i = 0; i < pPara_Modtcp->pcsnum; i++)
	{
